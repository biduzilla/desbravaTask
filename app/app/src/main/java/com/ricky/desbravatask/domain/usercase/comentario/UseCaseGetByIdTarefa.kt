...
class UseCaseGetByIdTarefa @Inject constructor(private val repository: ComentarioRepository) {
    operator fun invoke(id: String): Flow<Resource<List<Comentario>>> = flow {
        try {
            emit(Resource.Loading())
            repository.getByIdTarefa(id).let { result ->
