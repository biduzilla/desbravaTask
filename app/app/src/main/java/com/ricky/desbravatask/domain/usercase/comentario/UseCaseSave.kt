...
class UseCaseSave @Inject constructor(private val repository: ComentarioRepository) {
    operator fun invoke(comentario: Comentario): Flow<Resource<Comentario>> = flow {
        try {
            emit(Resource.Loading())
            repository.save(comentario).let { result ->
