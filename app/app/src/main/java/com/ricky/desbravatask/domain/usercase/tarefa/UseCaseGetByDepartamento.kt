...
class UseCaseGetByDepartamento @Inject constructor(private val repository: TarefaRepository) {
    operator fun invoke(id: String): Flow<Resource<List<Tarefa>>> = flow {
        try {
            emit(Resource.Loading())
            repository.getByDepartamento(id).let { result ->
